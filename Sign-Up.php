<?php 
session_start();
if($_SESSION['id'] != ''){header("Location: http://localhost/geniusparkle/dashboard.php");};
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<script>
var navCta = '<div id="nav-cta"><a data-btn="s" href="Sign-In.php">Log In</a><a data-btn="p" href="Sign-Up.php">Join Now</a></div>';
$("nav").append(navCta);
...
<section data-cntnr="md" data-pdg="xl">
  <h1 data-pdg="-sm">Join<br><span data-bg-anim="primary">GeniuSparkle</span></h1>
  <p data-pdg="-md" class="xl">
    It's quick and easy.
  </p>
  <form id="sign-up" action="reg.php" method="post">
    <div class="wrap-cntnr">
      <div data-input="txt">
        <label data-name="Full&#160;name">
          <input type="text" name="name" required minln="5" maxln="100" pattern="^(([a-zA-Z.'-]{1,50}){1}(\s([a-zA-Z'-]{1,50}))+)$">
        </label>
        <div class="error empty minln">
          Please enter your full name
        </div>
        <div class="error invalid maxln">
          Are you sure you entered your name correctly?
        </div>
      </div>
      <div data-input="txt">
        <label data-name="Email&#160;address">
        ...
        <div class="error regd">
          This email is already registered
        </div>
      </div>
      <div data-input="txt">
        <div style="display: grid; grid-template-columns: 80px 1fr 88px; grid-gap: 0 12px;">
          <label data-name="Day">
            <input type="number" name="dobDay" required min="1" max="31" maxlength="2">
          </label>
          <div class="stop-prop">
            <label data-name="Month">
              <input type="text" name="dobMonth" required readonly>
            </label>
            <ul class="ddl">
              <li>January</li>
              <li>February</li>
          ...
          <label data-name="Year">
            <input type="number" name="dobYear" required min="1900" max="2017" maxlength="4">
          </label>
        </div>
        <div class="guideline">
          Your date of birth
        ...
        <div class="error invalid min max minln maxln">
          Please enter a valid date
        </div>
      </div>
      <div data-input="txt">
        <label data-name="Password">
          <input type="password" name="password" required minln="8" maxln="100">
        </label>
        <div class="guideline">
        ...
        <div class="error minln">
          Use 8 characters or more for your password
        </div>
        <div class="error maxln">
          Use 100 characters or fewer for your password
        </div>
      </div>
    </div>
    <p class="sm clr-d" data-pdg="md-">
      By creating an account you agree to our <a href="Terms-of-Use">Terms of Use</a> and <a href="Privacy-Policy">Privacy Policy</a>.
    </p>
    <button data-btn="p" data-pdg="lg-">Join Now</button>
  </form>
  <p class="sm clr-d" data-pdg="xl-">
    Already have an account? <a href="/Sign-In">Log in</a>
  </p>
</section>
<script>
  $('form#sign-up button').on('click', function(event) {
    event.preventDefault();
    $('form#sign-up [data-input] input').each(function() {
      $.fn.validateInput(this);
    });
    $.fn.validateDate('dob');
    if (!$('form#sign-up').find('*').is('.err')) 
    return $('#sign-up').submit()
  });
</script>

<footer data-cntnr="xl" data-pdg="md">
